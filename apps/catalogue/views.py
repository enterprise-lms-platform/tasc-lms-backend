...
from .permissions import (
    CanEditBankQuestion,
    CanEditQuestionCategory,
    CanEditCourse,
    CanDeleteCourse,
    CanEditModuleCourse,
    CanEditSessionCourse,
    IsApprovalManager,
    IsCategoryManagerOrReadOnly,
    IsCourseWriter,
)
from .serializers import (
    ...
    ModuleSerializer,
    ModuleBulkReorderSerializer,
    QuestionCategorySerializer,
    QuizDetailSerializer,
    QuizQuestionListWriteSerializer,
    ...
    CourseApprovalRequestSerializer,
...
def _coerce_quiz_question_points(pts, existing_points=None):
    ...
    try:
        ...
        raise ValidationError(
            {"questions": [
                "Invalid points value; must be a non-negative integer."]}
        )
    if v < 0:
...
class QuestionCategoryViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated,
        IsCourseWriter, CanEditQuestionCategory]
    pagination_class = CataloguePageNumberPagination
...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                name="instructor", type=int, description="Filter by instructor ID"
            ),
            ...
            OpenApiParameter(
                name="is_published",
                type=bool,
                description="Filter by published status (true => published only, false => exclude published)",
            ),
        ],
    ...
    def perform_update(self, serializer):
        ...
        if old_status != instance.status:
            ...
            if (
                old_status != Course.Status.PUBLISHED
                and instance.status == Course.Status.PUBLISHED
            ):
    ...
    def submit_for_approval(self, request, pk=None):
        ...
        has_prior = (
            CourseApprovalRequest.objects.filter(course=course)
            .exclude(
                status=CourseApprovalRequest.Status.PENDING,
            )
            .exists()
        ...
        request_type = (
            CourseApprovalRequest.RequestType.EDIT
            if has_prior
            else CourseApprovalRequest.RequestType.CREATE
        )
    ...
    def destroy(self, request, *args, **kwargs):
        if getattr(request.user, "role", None) not in (
            ...
            raise PermissionDenied(
                "Only LMS Manager or TASC Admin can delete courses.")
...
class CourseApprovalRequestViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        comments = (serializer.validated_data.get(
            "reviewer_comments") or "").strip()
    ...
    def reject(self, request, pk=None):
        ...
        if not comments:
            raise ValidationError(
                {"reviewer_comments": ["A reason for rejection is required."]}
            )
...
class ModuleViewSet(viewsets.ModelViewSet):
        ...
        if self.action in ("update", "partial_update", "destroy"):
            ...
            if role == User.Role.INSTRUCTOR:
                queryset = queryset.filter(
                    course__instructor_id=self.request.user.id)
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                ...
                value={
                    "course": 5,
                    "order": [{"id": 10, "order": 0}, {"id": 11, "order": 1}],
                },
                request_only=True,
            ),
        ],
    ...
    def reorder(self, request):
        ...
        modules = list(Module.objects.filter(
            id__in=module_ids, course_id=course_id))
        if len(modules) != len(module_ids):
            raise ValidationError(
                "One or more modules do not belong to the specified course or do not exist."
            )
...
class SessionViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated,
        IsCourseWriter, CanEditSessionCourse]
    pagination_class = CataloguePageNumberPagination
    ...
    def _get_or_create_quiz(self, session):
        ...
        quiz, _ = Quiz.objects.get_or_create(
            session=session, defaults={"settings": {}})
        return quiz
    ...
    def perform_create(self, serializer):
        ...
        if role == User.Role.INSTRUCTOR and course.instructor_id != user.id:
            raise PermissionDenied(
                "You can only create sessions in your own courses.")
        serializer.save()
    ...
    def perform_update(self, serializer):
        ...
        if old_key and old_key != new_key:
            bucket = old_bucket or getattr(
                settings, "DO_SPACES_PRIVATE_BUCKET", "")
            if bucket:
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                name="type",
                type=str,
                description="Filter by session type (video, text, live)",
            ),
        ],
    )
    def list(self, request, *args, **kwargs):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                ...
                examples=[
                    OpenApiExample(
                        "Asset URL response",
                        value={
                            "url": "https://bucket.region.digitaloceanspaces.com/key?X-Amz-...",
                            "expires_in": 300,
                            "method": "GET",
                        },
                        response_only=True,
            ...
            403: OpenApiResponse(
                description="Not allowed to access this session asset"
            ),
            404: OpenApiResponse(
                description="Session has no asset or session not found"
            ),
    ...
    def asset_url(self, request, pk=None):
        ...
        bucket = (session.asset_bucket or "").strip() or getattr(
            settings, "DO_SPACES_PRIVATE_BUCKET", None
        )
        if not bucket:
            return Response(
    ...
    def submit(self, request, pk=None):
        ...
        enrollment = Enrollment.objects.filter(
            user=user, course=session.course).first()
        if not enrollment:
            return Response(
        ...
        if session.session_type == Session.SessionType.QUIZ:
            ...
            if not assignment:
                return Response(
                    {"detail": "Assignment not found for this session."},
                    status=status.HTTP_404_NOT_FOUND,
                )
    ...
    def _build_quiz_detail_response(self, session, quiz, request=None):
        ...
        if should_shuffle_answers:
            for q in questions:
                if q.question_type == QuizQuestion.QuestionType.MULTIPLE_CHOICE:
                    answer_payload = dict(q.answer_payload or {})
                    ...
                    if len(options) > 1:
                        ...
                        for new_idx, opt in enumerate(options):
                            opt["is_correct"] = new_idx in correct_indices
                        answer_payload["options"] = options
                    q.answer_payload = answer_payload
    ...
    def quiz(self, request, pk=None):
        ...
        if request.method == "GET":
            ...
            quiz = self._get_or_create_quiz(session)
            serializer = QuizSettingsUpdateSerializer(
                ...
                if qid is not None:
                    ...
                    if qid not in existing_ids:
                        raise ValidationError(
                            {
                                "questions": [
                                    f"Question id {qid} does not belong to this quiz."
                                ]
                            }
                        )
                    if qid in seen_ids:
                        raise ValidationError(
                            {"questions": [
                                f"Duplicate question id {qid} in payload."]}
                        )
                    seen_ids.add(qid)
                    ...
                    qobj.points = _coerce_quiz_question_points(
                        pts, existing_points=qobj.points
                    )
                    ...
                    QuizQuestion.objects.create(
                        ...
                        question_type=qt,
                        question_text=qtext,
                        points=_coerce_quiz_question_points(
                            pts, existing_points=None),
                        answer_payload=ap if isinstance(ap, dict) else {},
                    )
            to_delete = existing_ids - seen_ids
            if to_delete:
                QuizQuestion.objects.filter(
                    quiz=quiz, id__in=to_delete).delete()
    ...
    def add_from_bank(self, request, pk=None):
        ...
        with transaction.atomic():
            for bq in bank_questions:
                ...
                created.append(qq)
                next_order += 1
        questions = quiz.questions.filter(id__in=[q.id for q in created]).order_by(
            "order", "id"
        )
        return Response(
            {
    ...
    def assignment(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            return Response(
                AssignmentSerializer(
                    assignment).data, status=status.HTTP_201_CREATED
            )
...
class CourseReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = CourseReview.objects.filter(is_approved=True).select_related(
            "course", "user"
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"course": "Course not found."}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        existing = CourseReview.objects.filter(
            course=course, user=request.user).first()
        if existing:
            return Response(
        ...
        return Response(
            CourseReviewSerializer(review).data, status=status.HTTP_201_CREATED
        )
...
class CatalogueAnalyticsViewSet(viewsets.ViewSet):
    ...
    def courses_by_category(self, request):
        ...
        if user.role == "instructor":
            ...
            course_filter = Q(courses__instructor=user) & ~Q(
                courses__status="archived")
        ...
        data = [{"name": c.name, "count": c.course_count} for c in categories]
...
class SessionAttachmentViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not (is_owner or is_admin_like(user) or is_lms_manager(user)):
            raise PermissionDenied(
                "You do not have permission to add attachments to this course."
            )
        ...
        serializer.save(uploaded_by=user, file_size=file_size,
                        file_type=file_type)
