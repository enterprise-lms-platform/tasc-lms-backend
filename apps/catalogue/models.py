...
class LivestreamQuestion(models.Model):
    """
    Questions asked during livestream sessions.
    """
    ...
    answered_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name='answered_livestream_questions'
    )
    ...
    class Meta:
        ordering = ['-is_pinned', '-upvotes', '-asked_at']
        indexes = [
            models.Index(fields=['session', 'status']),
            models.Index(fields=['session', '-upvotes']),
        ]
        verbose_name = "Livestream Question"
        verbose_name_plural = "Livestream Questions"
    
    def __str__(self):
        return f"Q: {self.question[:50]}..."
    
    def mark_answered(self, answer, answered_by):
        """Mark question as answered"""
        self.answer = answer
        self.status = 'answered'
        self.answered_at = timezone.now()
        self.answered_by = answered_by
        self.save(update_fields=['answer', 'status', 'answered_at', 'answered_by'])
