...
from .serializers_superadmin import (
    OrganizationSuperadminSerializer,
    UserSuperadminSerializer,
...
class OrganizationSuperadminViewSet(viewsets.ModelViewSet):
    ...
    def export_csv(self, request):
        ...
        writer.writerow(
            ["ID", "Name", "Country", "City", "Contact Email", "Active", "Created At"]
        )
        for org in qs:
            writer.writerow(
                [
...
class UserSuperadminViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        suspended = qs.filter(is_active=False).count()
        by_role = list(qs.values("role").annotate(count=Count("id")).order_by("role"))
    ...
    def instructor_stats(self, request):
        ...
        avg_courses = (
            instructor_course_stats.aggregate(avg=Avg("course_count"))["avg"] or 0
        )
                ...
                with transaction.atomic():
                    ...
                    for user in users_to_create:
                        try:
                            ...
                            set_password_url = f"{frontend_base}/set-password/{uid}/{token}/"
                        
                            send_tasc_email(
                                subject="Welcome to TASC LMS - Set Your Password",
                                to=[user.email],
    ...
    def csv_template(self, request):
        ...
        writer.writerow(
            ["email", "first_name", "last_name", "role", "department", "phone_number"]
        )
        writer.writerow(
            [
                "example@domain.com",
    ...
    def export_csv(self, request):
        ...
        writer = csv_module.writer(response)
        writer.writerow(
            ["ID", "Email", "First Name", "Last Name", "Role", "Active", "Date Joined"]
        )
        for user in qs:
            writer.writerow(
                [
...
class SecurityStatsView(APIView):
    ...
    def get(self, request):
        ...
        mfa_enabled = (
            User.objects.filter(is_active=True, is_mfa_enabled=True).count()
        ...
        # Count active sessions (last activity within last 24 hours)
        one_day_ago = now - timedelta(hours=24)
        active_sessions = UserSession.objects.filter(
            is_active=True, last_activity__gte=one_day_ago
        ).count()
...
class UserSessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if recent_only and recent_only.lower() == 'true':
            qs = qs.filter(last_activity__gte=one_day_ago)
    ...
    def terminate_all(self, request):
        ...
        UserSession.objects.filter(
            is_active=True, 
            last_activity__gte=one_day_ago
...
class UserGrowthStatsView(APIView):
    ...
    def get(self, request):
        ...
        prev_signups = User.objects.filter(
            date_joined__gte=prev_start, date_joined__lt=period_start
        ).count()

        # Active users (logged in last 30 days)
        curr_active = User.objects.filter(
            last_login__gte=period_start, is_active=True
        ).count()
        prev_active = User.objects.filter(
            last_login__gte=prev_start, last_login__lt=period_start, is_active=True
        ).count()

        # Returning users (logged in this period, joined before this period)
        curr_returning = User.objects.filter(
            last_login__gte=period_start, date_joined__lt=period_start, is_active=True
        ).count()
        prev_returning = User.objects.filter(
            last_login__gte=prev_start,
            last_login__lt=period_start,
            date_joined__lt=prev_start,
            is_active=True,
        ).count()

        # Churned: active in prev window but not in current window
        curr_churned = (
            User.objects.filter(last_login__gte=prev_start, last_login__lt=period_start)
            .exclude(last_login__gte=period_start)
            .count()
        )
        pre_prev_start = now - timedelta(days=90)
        prev_churned = (
            User.objects.filter(
                last_login__gte=pre_prev_start, last_login__lt=prev_start
            )
            .exclude(last_login__gte=prev_start)
            .count()
        )
                ...
                "metrics": [
                    {
                        "label": "New Signups",
                        "value": curr_signups,
                        "change": self._pct_change(curr_signups, prev_signups),
                        "positive": self._pct_change(curr_signups, prev_signups) >= 0,
                    },
                    {
                        "label": "Active Users",
                        "value": curr_active,
                        "change": self._pct_change(curr_active, prev_active),
                        "positive": self._pct_change(curr_active, prev_active) >= 0,
                    },
                    {
                        "label": "Returning Users",
                        "value": curr_returning,
                        "change": self._pct_change(curr_returning, prev_returning),
                        "positive": self._pct_change(curr_returning, prev_returning)
                        >= 0,
                    },
                    {
                        "label": "Churned",
...
class SuperadminReviewViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        qs = CourseReview.objects.select_related("course", "user").order_by(
            "-created_at"
        )
    ...
    def post(self, request):
        ...
        try:
            send_mail(
                ...
                from_email=getattr(
                    django_settings, "DEFAULT_FROM_EMAIL", "noreply@tasc.co.ug"
                ),
...
class SuperadminAssessmentsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
                ...
                results.append(
                    {
                        ...
                        if quiz.session and quiz.session.course
                        else "",
        ...
        if assessment_type != "quiz":
            for asn in Assignment.objects.select_related(
                ...
                results.append(
                    {
                        ...
                        if asn.session and asn.session.course
                        else "",
    ...
    def stats(self, request):
        ...
        pass_rate = (
            round((passed / total_attempts * 100), 1) if total_attempts > 0 else 0.0
        )
        return Response(
            {
                "total": total,
